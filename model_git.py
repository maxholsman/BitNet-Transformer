...
class EncoderBlock(nn.Module):
    def __init__(self, d_model, self_attention_block, ffblock, dropout=0.1):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.self_attention_block = self_attention_block
        self.ffblock = ffblock
        
        self.residual_connections = nn.ModuleList([ResidualConnection(d_model, dropout) for _ in range(2)])
        
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, src_mask))
        x = self.residual_connections[1](x, self.ffblock)
        return x
...
class DecoderBlock(nn.Module):
    def __init__(self, d_model, self_attention_block, cross_attention_block, ffblock, dropout=0.1):
        super().__init__()
        self.self_attention_block = self_attention_block
    ...
    def forward(self, x, encoder_output, src_mask, target_mask):
        ...
        x = self.residual_connections[2](x, self.ffblock)
        return x
